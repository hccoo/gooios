...
namespace Gooios.GoodsService.Repositories
{
    public class Repository<E, K> : IRepository<E, K> where E : Entity<K>
    {
        ...
        {
            _databaseContext = dbContextProvider.GetDbContext();
            _dbset = _databaseContext.Set<E>();
        }
        ...
        public IEnumerable<E> GetPaged<KProperty>(int pageIndex, int pageCount, Expression<Func<E, bool>> whereExpression, Expression<Func<E, KProperty>> orderByExpression, bool ascending)
        {
            if (ascending)
            {
                return _dbset.Where(whereExpression).OrderBy(orderByExpression)
                          .Skip(pageCount * (pageIndex - 1))
                          .Take(pageCount);
            }
            else
            {
                return _dbset.Where(whereExpression).OrderByDescending(orderByExpression)
                          .Skip(pageCount * (pageIndex - 1))
                          .Take(pageCount);
            }
        }
